...
class PlateOCR(NvDsPyFuncPlugin):
    ...
    def __init__(
        self,
        plate_model_path: str = "/models/yolov8n_plate/yolov8n_plate.onnx",
    ...
    def on_start(self) -> bool:
        ...
        try:
            ...
            # Degrade gracefully: keep the pipeline running even if OCR deps/models
            # are not available in the current environment.
            logger.error("PlateOCR disabled (on_start failed): %s", exc, exc_info=True)
            self._disabled = True
            return True
    ...
    def _init_models(self):
        ...
        # albumentations (PaddleOCR dependency) imports paths that reference cv2 inpaint
        # constants; some DeepStream/system cv2 builds omit these module-level attrs.
        if not hasattr(cv2, "INPAINT_TELEA"):
            cv2.INPAINT_TELEA = 1
        if not hasattr(cv2, "INPAINT_NS"):
            cv2.INPAINT_NS = 2

        use_gpu_env = os.getenv("PADDLEOCR_USE_GPU", "0").strip().lower()
        use_gpu = use_gpu_env in {"1", "true", "yes", "y", "on"}

        # Defensive: even with PaddleOCR(use_gpu=False), Paddle GPU wheels may try to
        # touch CUDA during import/initialization on some setups. Force CPU unless
        # explicitly enabled.
        if not use_gpu:
        ...
        logger.info("Initializing PaddleOCR (lang=en, use_gpu=%s)...", use_gpu)
        self._ocr = PaddleOCR(
        ...
        # Warm up PaddleOCR (GPU init can be expensive on first call).
    ...
    def process_frame(self, buffer, frame_meta: NvDsFrameMeta) -> None:
        ...
        try:
            metrics.frames_processed_total.labels(source_id=str(frame_meta.source_id)).inc()
        except Exception:
            pass
        ...
        for obj_meta in vehicle_objects:
            ...
            try:
                with metrics.aicore_inference_ms.labels(camera_id=source_id, model="plate_detector").time():
                    plates = self._detect_plates(vehicle_crop)
            except Exception as exc:
                logger.warning("Plate detection failed for source=%s: %s", source_id, exc)
                plates = []
            if not plates:
            ...
            if _sharpness(plate_crop) < _MIN_PLATE_SHARPNESS:
                try:
                    metrics.lpr_ocr_total.labels(camera_id=source_id, result="low_quality").inc()
                except Exception:
                    pass
                continue

            try:
                with metrics.aicore_inference_ms.labels(camera_id=source_id, model="ocr").time():
            ...
            if plate_text and ocr_conf >= self.ocr_conf_threshold:
                ...
                # Telemetry: OCR success + LPR event produced
                try:
                    metrics.lpr_ocr_total.labels(camera_id=source_id, result="success").inc()
                    metrics.lpr_events_total.labels(camera_id=source_id).inc()
                except Exception:
                    pass
                ...
                try:
                    metrics.lpr_ocr_total.labels(camera_id=source_id, result="ocr_low_confidence").inc()
                except Exception:
                    pass
