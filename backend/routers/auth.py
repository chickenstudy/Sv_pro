...
# ── Cấu hình ───────────────────────────────────────────────────────────────────
_JWT_SECRET    = os.environ.get("JWT_SECRET", "change-me-in-production-please")
_JWT_ALGO      = "HS256"
_JWT_EXP_HOURS = 24
...
def _create_token(username: str) -> str:
    ...
    try:
        ...
        raise RuntimeError("Cài đặt pyjwt: pip install pyjwt")
...
# Tài khoản đơn giản lưu trong env (thay bằng DB trong production)
_ADMIN_USER = os.environ.get("ADMIN_USERNAME", "admin")
_ADMIN_PASS = os.environ.get("ADMIN_PASSWORD", "svpro2024")
...
async def login(body: LoginRequest):
    ...
    """
    # Kiểm tra đơn giản (production nên hash password + query DB)
